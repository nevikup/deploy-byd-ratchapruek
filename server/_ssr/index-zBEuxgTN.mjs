...
import { s as Route$5, A as ARTICLES, P as PageShell, t as heroImg, H as HOME_SLIDES_ENDPOINT, u as extractSlideImageUrls, v as extractLargestSrcsetUrl, w as normalizeImageUrl } from "./router-DRa5uQgx.mjs";
...
import "./models-0Rvkj58o.mjs";
import "../_libs/clsx.mjs";
import "../_libs/tailwind-merge.mjs";
import "./server-CJtspEm5.mjs";
...
const ModelsScroller = reactExports.lazy(() => import("./ModelsScroller-CLROio1C.mjs").then((module) => ({
...
function extractPopupContent(payload) {
  ...
  const contentDocument = new DOMParser().parseFromString(renderedContent, "text/html");
  const image = contentDocument.querySelector("img");
  if (!image) {
    return null;
  }
  const imgTag = image.outerHTML;
  const imageUrlFromSrcset = extractLargestSrcsetUrl(imgTag);
  const imageUrlFromSrc = normalizeImageUrl(image.getAttribute("src")?.trim() ?? "");
  ...
  const rawLinkUrl = image.closest("a[href]")?.getAttribute("href")?.trim() ?? "";
  let linkUrl = null;
  if (rawLinkUrl && rawLinkUrl !== "#") {
    try {
      const parsedLinkUrl = new URL(rawLinkUrl, window.location.href);
      if (parsedLinkUrl.protocol === "http:" || parsedLinkUrl.protocol === "https:") {
        linkUrl = parsedLinkUrl.href;
      }
    } catch {
      linkUrl = null;
    }
...
function HomePopupModal() {
  ...
  const [popupContent, setPopupContent] = reactExports.useState(null);
  reactExports.useEffect(() => {
    ...
    async function loadPopupImage() {
      try {
        ...
        const content = extractPopupContent(payload);
        if (!content) {
          return;
        }
  ...
  if (!popupContent || !isOpen) {
    return null;
  }
  const popupImage = /* @__PURE__ */ jsxRuntimeExports.jsx("img", { src: popupContent.imageUrl, alt: "Popup promotion", className: "mx-auto block h-auto max-h-[calc(100dvh-8rem)] w-full object-contain", loading: "lazy", decoding: "async" });
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "fixed inset-0 z-[80] flex items-center justify-center bg-black/65 px-3", role: "dialog", "aria-modal": "true", "aria-label": "Popup Promotion", onClick: () => setIsOpen(false), children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "popup-card-enter w-auto max-h-[calc(100dvh-1rem)] max-w-[calc(100vw-1.5rem)] overflow-y-auto p-0", onClick: (event) => event.stopPropagation(), children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "relative w-[min(398px,calc(100vw-1.5rem))] overflow-hidden rounded-2xl border border-white/25 bg-black/85 shadow-[0_24px_80px_rgba(0,0,0,0.55)] backdrop-blur-md sm:w-[min(590px,calc(100vw-3rem))]", children: popupContent.linkUrl ? /* @__PURE__ */ jsxRuntimeExports.jsx("a", { href: popupContent.linkUrl, target: "_blank", rel: "noopener noreferrer", children: popupImage }) : popupImage }),
...
function HomePage() {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx(PageShell, { children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "overflow-x-clip", children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "relative min-h-screen overflow-hidden", children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "relative mx-auto flex min-h-screen w-full max-w-7xl items-center px-4 py-32 sm:px-6 lg:px-8", children: [
        "          ",
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "py-24 px-6", children: [
