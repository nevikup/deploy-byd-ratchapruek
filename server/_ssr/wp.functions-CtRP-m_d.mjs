import { c as createServerRpc } from "./createServerRpc-BvkQYLys.mjs";
import { c as createServerFn } from "./server-Cf8QH-h8.mjs";
import { r as resolveInstallmentCoverImageFromTitle } from "./models-0Rvkj58o.mjs";
...
function decodeHtmlEntities(value) {
  return value.replace(/&#(\d+);/g, (_, dec) => String.fromCodePoint(Number(dec))).replace(/&#x([\da-f]+);/gi, (_, hex) => String.fromCodePoint(parseInt(hex, 16))).replace(/&amp;/gi, "&").replace(/&lt;/gi, "<").replace(/&gt;/gi, ">").replace(/&quot;/gi, '"').replace(/&#39;/g, "'").replace(/&hellip;/gi, String.fromCodePoint(8230)).replace(/&nbsp;/gi, " ");
}
